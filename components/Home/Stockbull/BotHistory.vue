<template>
  ...
  <div v-if="botHistory.wallets[0] > 0">
    ...
    <div class="w-full flex p-2 justify-end ">
      ...
      <UButton @click="switchModal = !switchModal" variant="solid">CHANGE MARKET</UButton>
    </div>
    <HomeStockbullSwitchChart @closeModal="switchModal = false" :showModal="switchModal"></HomeStockbullSwitchChart>
    <div>
      <div className="flex-col flex  gap-y-2 relative  max-h-[400px]  overflow-y-scroll py-4">
        <div v-for="(item, index) in botHistory.activities" :key="index"
          ...
          <div className="w-full h-1 rounded-md overscroll-none bg-slate-900 ">
            <div :style="`background-color: ` + (item.type == 0 ? 'red' : 'green')" class="w-[100%] rounded-md h-full">
            </div>
          </div>
        </div>
      </div>
