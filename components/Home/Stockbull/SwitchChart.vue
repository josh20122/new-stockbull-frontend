<template>
  ...
  >
    ...
    >
      <template #header>
        <div class="text-center">SHIFT MARKET</div>
      </template>
      ...
      <div class=" ">
        ...
        <div class="text-sm pt-5">
          <div class="pb-4">
            Are you sure you want to shift from
            {{ chartAttributes[form.from] }} market to
            {{ chartAttributes[form.to] }} market now? Note the following:
          </div>
          <ol class="flex flex-col gap-y-2 pl-2">
            <li>
              1. All the data in {{ chartAttributes[form.from] }} market will be
              deleted.
            </li>
            <li>
              2. All the data will be shifted to
              {{ chartAttributes[form.to] }} market.
            </li>
            <li>
              3. No more profits accumulation from
              {{ chartAttributes[form.from] }} markets will be added.
            </li>
      ...
      <template #footer>
        <div class="flex gap-x-3 w-full justify-start">
          <UButton @click="modalIsOpen = false" color="red" variant="solid"
            >CANCEL</UButton
          >
...
const submitForm = () => {
  axios
    .post("/shift-chart", form.value)
    .then((response) => {
      handleNotification(
        `You have successfully shifted from ${
          chartAttributes[form.value.from]
        } to ${chartAttributes[form.value.to]}.`
      );
    })
    .catch((err) => {
...
watch(
  () => activestockbullChart.value.name,
  (newValue) => {
